...
class Solution:
	...
	def sumBitDifferences(self,arr, n):
    	# code here
            ...
            # Initialize an array to store the count of set bits at each position
            bit_count = [0] * 32
        
            # Count the set bits at each position for all elements in the array
            for num in arr:
                for i in range(32):
                    bit_count[i] += (num >> i) & 1
        
            # Calculate the sum of bit differences using the counts
            for count in bit_count:
                sum_result += count * (n - count) * 2
